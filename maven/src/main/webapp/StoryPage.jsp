...
#scrollBtn:hover, #productDetailBtn:hovoer {
...
<body>
	...
	<div class="container">
		...
		<div class="intro">
			<h2><%=x.getFh_nick()%></h2>
			<p>
				농장주 <strong><%=x.getFh_owner()%></strong>님은 항상 자연을 생각하며 농사를 지으십니다.<br>
				<strong><%=x.getFh_name()%></strong> 농장은
				<%=x.getFh_intro()%>.
			</p>
		</div>
		...
		<div class="crops">
			...
			<p>
				저희 농장은
				<%=x.getFh_region()%>에서 재배를 하고 있으며 재배하는 농산물은 다음과 같습니다:
			</p>
			<h2>농산품 상세 정보</h2>
			<%
			...
			<div id="product-detail" class="product-detail">
				...
				%>
				<p>
					우리 농장의 <strong><%=product.getAgri_name()%></strong>는
					<%=product.getEffect()%>의 효능이 있습니다.
				</p>
				<%
				}
				...
				<p>
					우리 농장의 <strong><%=product.getAgri_name()%></strong>는
					<%=product.getTime_production()%>기간에 나며
				</p>
				<%
				}
				if (product.getTrimming() != null) {
				%>
				<p><%=product.getTrimming()%>
					방법으로 손질하면
				</p>
				<%
				}
...
<button onclick="scrollToTop()" id="scrollBtn" title="위로 이동">맨위로이동</button>
<button id="productDetailBtn" title="농산품 상세 정보로 이동">농산품 상세 정보로 이동</button>
